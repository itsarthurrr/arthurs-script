...
local flying, noclip, aimbotEnabled, xrayEnabled, hitboxEnabled, wallHopEnabled, infJumpEnabled, wallGlideEnabled, fakeLagEnabled, lagSwitchEnabled = false, false, false, false, false, false, false, false, false, false
local flySpeed = 80
...
local function createSlider(name, min, max, default, pos)
    ...
    RunService.RenderStepped:Connect(function()
        if dragging then
            ...
            label.Text = name .. ": " .. val
            if name == "Walk Speed" then walkSpeedValue = val end
            if name == "Jump Power" then jumpPowerValue = val end
        end
    end)
...
local function runPshade() loadstring(game:HttpGet('https://raw.githubusercontent.com/randomstring0/pshade-ultimate/refs/heads/main/src/cd.lua'))() end
...
local psBtn, psInd = createToggle("PShade", "P", runPshade)
...
UIS.InputBegan:Connect(function(input, gpe)
    ...
    elseif input.KeyCode == Enum.KeyCode.P then runPshade()
...
task.spawn(function()
    while task.wait() do
        pcall(function()
            ...
            -- Förbättrad Fake Lag (mindre laggig, bättre för MM2)
            if fakeLagEnabled then
                local root = player.Character:FindFirstChild("HumanoidRootPart")
                if root then
                    root.Anchored = true
                    task.wait(0.04) -- Mycket kortare delay
                    root.Anchored = false
                end
            end
        end)
...
RunService.Stepped:Connect(function()
    ...
    if (wallHopEnabled or wallGlideEnabled) then
        ...
        if part and part.CanCollide then
            if wallHopEnabled and UIS:IsKeyDown(Enum.KeyCode.Space) then
                root.Velocity = Vector3.new(root.Velocity.X, jumpPowerValue, root.Velocity.Z)
            end
    ...
    if flying then
        local bv = root:FindFirstChild("XenoFlyVel") or Instance.new("BodyVelocity", root)
        bv.Name = "XenoFlyVel"; bv.MaxForce = Vector3.new(9e9, 9e9, 9e9)
        local bg = root:FindFirstChild("XenoFlyGyro") or Instance.new("BodyGyro", root)
        bg.Name = "XenoFlyGyro"; bg.MaxTorque = Vector3.new(9e9, 9e9, 9e9); bg.CFrame = camera.CFrame
